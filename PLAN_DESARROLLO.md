...
> **REVISIÓN POST-COMPLETADO (Paso 2.3): imagen de producto por URL externa, no por archivo subido al servidor.** Decisión del propietario: en vez de `FileUpload` (que sube el archivo y lo guarda en el disco del proyecto), `imagen_principal` se llena pegando una URL de una imagen ya alojada en un servicio externo en la nube (ej. Cloudinary u otro que use el equipo). Cambiar el campo del formulario de `FileUpload` a un `TextInput` validado con `->url()`, con **vista previa en vivo** (`->live()` + un elemento que renderice `<img src="{{ URL pegada }}">` debajo, actualizado mientras se escribe) para que el admin confirme visualmente que la imagen es la correcta antes de guardar. No requiere ningún cambio de esquema (`imagen_principal` ya era `varchar(255)`, pensado para ruta o URL desde el inicio). Ya no aplica la regla 15 de `CLAUDE.md` (`storage:link`) a este campo específico, porque no hay archivo que Laravel gestione localmente.
...
- [x] **Paso 3.3: Interfaz:** Tablas en el panel `almacen` para ver el stock actual y gestionar/aprobar traspasos entrantes; en el panel `pos`, solo consulta de stock + creación de solicitudes de traspaso por el Vendedor. **Implementado:** `AislaPorAlmacen` (nuevo Trait) + `InventarioResource`/`TraspasoResource` en el panel `almacen` con transiciones de estado y `CompletarTraspasoAction`; `InventarioResource`/`TraspasoResource` en el panel `pos` con `AislaPorSucursal`, formulario de solicitud (Repeater) y `SolicitarTraspasoAction`; `InventarioPolicy`/`TraspasoPolicy` nuevas. 7 tests de aislamiento/UI (27/27 en la suite default).

> **RESUELTO (Paso 3.3 ahora sí cerrado del todo):** el selector de "almacén destino" en `App\Filament\Pos\Resources\Traspasos\Schemas\TraspasoForm` tenía dos huecos: (a) el `->options()` solo hacía `pluck('nombre', 'id')`, sin mostrar a qué sucursal pertenecía cada almacén, y (b) filtraba solo por `sucursal_id`, sin exigir `tipo = 'tienda'` — un Vendedor podía en teoría elegir como destino un almacén `deposito` de su propia sucursal. Corregido: ahora el `->options()` filtra explícitamente por `tipo = AlmacenTipo::Tienda` además de `sucursal_id`, y el label de cada opción incluye el nombre de la sucursal (`"{$almacen->sucursal->nombre} — {$almacen->nombre}"`). Verificado con un test nuevo (`test_vendedor_no_puede_elegir_como_destino_un_almacen_de_tipo_deposito_de_su_propia_sucursal` en `tests/Feature/InventarioTraspasoAislamientoUiTest.php`) que confirma que un depósito de la misma sucursal es rechazado por validación de formulario. Suite completa: 28/28.

- [x] **Paso 3.4: Ajuste Manual de Inventario (hueco detectado — necesario antes de probar Fase 4):** interfaz simple en el panel `admin` (Resource o Page, no importa cuál mientras sea `admin`-only) que invoque el `InventoryAction` ya construido en 3.2 con `tipo_movimiento = 'ajuste'`, capturando almacén, producto_variante, cantidad (+/-) y motivo obligatorio — es la única vía real para dar stock inicial a un producto antes de que exista Compras (Fase 5). Ver `LOGICA_NEGOCIO.md` sección 5 ("Ajustes Manuales: Restringidos al Administrador"). **Nota:** `LOGICA_NEGOCIO.md` también menciona "o el Almacenero con previa autorización" — ese mecanismo de autorización nunca se definió técnicamente; queda fuera de alcance por ahora (solo `admin` puede ajustar), se revisita si el negocio lo pide explícitamente.
    - **Implementado:** `App\Filament\Pages\AjusteInventario` — Page simple (no Resource, no hay listado/edición de ajustes, son inmutables) registrada solo en `App\Filament\Resources`/`App\Filament\Pages` del panel `admin`, con `canAccess()` explícito (`Auth::user()?->isAdmin()`) además de la restricción implícita de `canAccessPanel()`. Un único header `Action` ("Registrar ajuste") abre un modal con: `almacen_id` (Select, todos los almacenes, label con formato "Sucursal — Almacén (Tipo)"), `producto_variante_id` (Select buscable), `cantidad` (`TextInput::integer()` + regla `not_in:0`) y `motivo` (Textarea, `required`, `maxLength(100)` acorde a la columna). El `->action()` invoca `RegistrarMovimientoInventarioAction::handle()` con `tipo_movimiento = Ajuste`; si lanza `StockInsuficienteException` (ajuste negativo que dejaría el stock en negativo), se captura, se notifica el error y se hace `$action->halt()` para no cerrar el modal ni perder los datos capturados. Verificado con 6 tests nuevos en `tests/Feature/AjusteInventarioPageTest.php` (403 para no-admin, 200 para admin, ajuste positivo actualiza stock y Kardex, ajuste negativo que excede stock se rechaza sin tocar `inventarios`, motivo obligatorio, cantidad no puede ser cero). Suite completa: 34/34.

> **PENDIENTE (no bloquea Fase 4, pero anotado):** el panel `admin` no tiene ninguna vista de Inventario/Traspaso, pese a que `LOGICA_NEGOCIO.md` sección 2 define al Administrador con "visión panorámica de todos los almacenes y sucursales". Falta una vista de solo lectura (sin transiciones de estado) para cumplir esa definición de rol. Se agrega cuando convenga, no es urgente.
