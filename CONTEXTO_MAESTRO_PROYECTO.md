...
>
> **Última actualización:** Fase 3 (Motor de Inventario y Traspasos) completa, con 2 pendientes detectados antes de arrancar Fase 4 (verificación de scoping del selector de almacén destino en traspasos, y Paso 3.4 — Ajuste Manual de Inventario, aún no construido).
>
...
- ✅ **Fase 3** completa (Kardex: `inventarios`, `movimientos_inventario`, `traspasos`, `traspaso_detalles`, `almacen_usuario`; `InventoryAction` con `lockForUpdate()`; `AislaPorAlmacen` nuevo Trait; Resources de Inventario/Traspaso en paneles `almacen` y `pos`; Policies nuevas; 27/27 tests pasando).
  - ⏳ **Pendiente antes de considerar la Fase 3 100% cerrada:**
    1. **Verificar** que el selector de "almacén destino" en la solicitud de traspaso (panel `pos`) muestra la sucursal de cada almacén y restringe las opciones solo al almacén `tienda` de la sucursal del Vendedor autenticado.
    2. **Paso 3.4 (nuevo, hueco detectado):** construir "Ajuste Manual de Inventario" en el panel `admin` — reutiliza el `InventoryAction` ya existente, es la única forma de dar stock real a un producto antes de que exista Compras (Fase 5). Necesario para poder probar la Fase 4 con datos reales.
  - ⏳ **Pendiente sin urgencia (no bloquea):** el panel `admin` no tiene ninguna vista de Inventario/Traspaso, pese a que `LOGICA_NEGOCIO.md` define al Administrador con "visión panorámica de todos los almacenes y sucursales". Se agrega cuando convenga.
...
* **Pruebas de concurrencia — dos niveles, no uno:** invariante de negocio en la suite normal (SQLite) + prueba de bloqueo de fila real en grupo separado (`@group concurrency`) contra Postgres real, porque SQLite trata `lockForUpdate()` como no-op. Aplica y se repite en Fase 3 (ya usado), Fase 4 (cierre de venta, canje de puntos).
...
6. **Huecos entre teoría (`LOGICA_NEGOCIO.md`) y plan técnico (`PLAN_DESARROLLO.md`)** se cierran agregando un paso accionable, no solo un comentario suelto (así se corrigieron: permisos por rol, aislamiento por sucursal, aislamiento por almacén, y ahora Ajuste Manual de Inventario / vista global del admin).
...
* Pide "regenera el CONTEXTO_MAESTRO_PROYECTO.md" cada vez que se acumulen varias decisiones nuevas.
